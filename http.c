...
{
        write_buf_format(sess, "HTTP/1.1 %s\r\n", get_status_text(code));
        write_buf_format(sess, "Date: %s\r\n", get_http_fmt_date(time(NULL)));
        write_buf_format(sess, "Server: Tiny HTTP Server (Unix)\r\n");
        write_buf_format(sess, "Connection: close\r\n");
}

void http_content_headers(struct session *sess, const char *type,
                          size_t length, time_t mt)
{
        write_buf_format(sess, "Accept-Ranges: none\r\n");
        write_buf_format(sess, "Content-Type: %s\r\n", type);
        write_buf_format(sess, "Content-Length: %ld\r\n", length);
        write_buf_format(sess, "Last-Modified: %s\r\n", get_http_fmt_date(mt));
}

void http_crlf(struct session *sess)
{
        write_buf_format(sess, "\r\n");
}

void *http_check_request_end(const char *buf, int size)
{
        return memcmp(buf + size - 4, "\r\n\r\n", 4) ? NULL : (void *)buf;
}
