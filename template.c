...
void write_buf_format(struct session *sess, const char *fmt, ...)
{
        va_list vl;
        va_start(vl, fmt);
        sess->sbuf_used += vsnprintf(sess->sendbuf + sess->sbuf_used,
                                     OUTBUFSIZE - sess->sbuf_used, fmt, vl);
        va_end(vl);
}

void write_fbuf_format(struct session *sess, const char *fmt, ...)
{
        va_list vl;
        va_start(vl, fmt);
        sess->tx_len += vsnprintf(sess->tx_buf + sess->tx_len,
                                  TX_BUF_SIZE - sess->tx_len, fmt, vl);
        va_end(vl);
}
